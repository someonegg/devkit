...
- ExecPlans must contain and maintain a `Progress` section, a `Review Scope` section, a `Surprises & Discoveries` section, a `Decision Log`, and an `Outcomes & Retrospective` section. These are not optional.
...
Define exactly what Review must cover. Keep this section current during implementation so reviewers know the precise target.
